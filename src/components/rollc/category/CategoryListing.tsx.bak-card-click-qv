                  ...
                  return (
                    <article className={styles.card} key={product.id}>
                      <div className={styles.cardMedia}>
                        ...
                        <button type="button" className={`${styles.cardFav} ${isFav ? styles.cardFavActive : ""}`} onClick={() => toggleFav(product)} aria-label={locale === "ar" ? "المفضلة" : "Wishlist"}>
                          <svg viewBox="0 0 24 24"><path d="M12 20s-7-4.5-7-9.5A3.8 3.8 0 0 1 12 7a3.8 3.8 0 0 1 7 3.5C19 15.5 12 20 12 20Z" /></svg>
                        </button>
                        <a href={href(product.id)} aria-label={product.name[locale]}>
                          <img src={product.img} alt={product.name[locale]} loading="lazy" />
                        </a>
                        <button type="button" className={styles.quick} onClick={() => openQuickView(product)}>
                          {locale === "ar" ? "عرض سريع" : "Quick view"}
                        </button>
                      </div>
                      <div className={styles.cardBody}>
                        <span className={styles.cardCat}>{product.cat[locale]}</span>
                        <a href={href(product.id)}><h3 className={styles.cardName}>{product.name[locale]}</h3></a>
                        ...
                        <div className={styles.cardFoot}>
                          ...
                          <button type="button" className={styles.addCart} onClick={() => handleAdd(product)} aria-label={locale === "ar" ? "أضف" : "Add"}>
